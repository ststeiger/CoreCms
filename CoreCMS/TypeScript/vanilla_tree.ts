...
namespace VT
{
    ...
    export class VanillaTree
    {
        ...
        public async addBranch(id:string) :Promise<ITreeData>
        {
            let data:ITreeData = null;
            
            try
            {
                // Http.URL.Parameters["uid"];
                data = await new Http.PostJSON("treedata", { "id": id }).sendAsync();
                // window["foo"] = data;
        ...
        public async open(id: string): Promise<VanillaTree> 
        {
            ...
            let children = leaf.getElementsByTagName("ul") || [];
