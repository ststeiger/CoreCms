...
// https://docs.microsoft.com/en-us/dotnet/api/microsoft.aspnetcore.server.kestrel.internal.system.readonlyspan-1?view=aspnetcore-2.0
// system.memory/System instead of kestrell... removed ReadOnlySpan
//    https://github.com/dotnet/corefx/blob/master/src/System.Memory/src/System/ReadOnlySpan.cs
//    https://raw.githubusercontent.com/dotnet/corefx/master/src/Common/src/System/Security/Cryptography/DerEncoder.cs
    ...
    internal static class DerEncoder
    {
        ...
        internal static byte[][] SegmentedEncodeOid(string oidValue)
        {
            ...
            if (string.IsNullOrEmpty(oidValue))
                throw new CryptographicException("Invalid Oid-value");
        ...
        internal static byte[][] SegmentedEncodeIA5String(char[] chars, int offset, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (c > 127)
                {
                    throw new CryptographicException("Invalid IA5-string");
                }
